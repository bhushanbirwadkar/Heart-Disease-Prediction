...
def user_input_features():
    ...
    fbs = st.selectbox("(1= If FBS > 120 mg/dl, 0= FBS < 120 mg/dl)",["1","0"])
    st.write("""**You selected this option **""",fbs)
    
    st.write("""**7. Select Resting electrocardiographic results :**""")
    restecg = st.selectbox("(0 = Normal, 1 = having ST-T wave abnormality (T and/or ST elevation or depression of > 0.05 mV), 2 = Showing probable or definite left ventricular hypertrophy by Estes' criteria) : ",["0","1","2"])
    st.write("""**You selected this option **""",restecg)
    
    st.write("""**8. Maximum Heart Rate Achieved (THALACH) :**""")
    ...
    slope = st.selectbox("(Select 0=Upsloping, 1=Flat or 2=Downsloping)",["0","1","2"])
